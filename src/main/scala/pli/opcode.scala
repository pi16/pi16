...
object Opcode {
  ...
  /** The opcode for the `zor` instruction.
    *
    * The `zor` instruction pops two boolean from the stack and
    * then pushes the result of the disjunction the stack.
    */
  val zor = 13
  
  /** The opcode for the `zoror` instruction.
    *
    * The `zoror` instruction pops two numbers from the stack and
    * then pushes the result of the disjunction on the stack.
    */
  val zoror = 14
  
  /** The opcode for the `zand` instruction.
    *
    * The `zand` instruction pops two numbers from the stack and
    * then pushes the result of the conjuntion on the stack.
    */
  val zand = 15
  
  /** The opcode for the `zandand` instruction.
    *
    * The `zandand` instruction pops two numbers from the stack and
    * then pushes the result of the conjunction on the stack.
    */
  val zandand = 16
  
  /** The opcode for the `equalsequals` instruction.
    *
    * The `equalsequals` instruction pops two numbers from the stack and
    * then pushes the result of the equivalence on the stack.
    */
  val equalsequals = 17
  
    /** The opcode for the `true` value.
    *
    * The `true` value
    */
  val ztrue = 18
  
    /** The opcode for the `false` value.
    *
    * The `false` value
    */
  val zfalse = 19
  ...
  val values =
    Seq(exit, iadd, isub, imul, iconst, goto, ifeq, ifne, print,
      iload, istore, pop, zor, zoror, zand, zandand, equalsequals, ztrue, zfalse,zload, zstore)
  ...
  def toString(opcode: Int): String =
    opcode match {
      ...
      case Opcode.pop => "pop"
      case Opcode.zor => "zor"
      case Opcode.zoror => "zoror"
      case Opcode.zand => "zand"
      case Opcode.zandand => "zand"
      case Opcode.equalsequals => "equalsequals"
      case Opcode.ztrue => "ztrue"
      case Opcode.zload => "zload"
      case Opcode.zstore => "zstore"
      case Opcode.zfalse => "zfalse"
    }
